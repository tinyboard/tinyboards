...
use tinyboards_auth::configure_auth_routes_with_secret;
...
async fn main() -> Result<(), TinyBoardsError> {
    ...
    let secret = {
        ...
        use tinyboards_db::schema::secrets;
        let mut conn = pool.get().await.expect("Couldn't get DB connection for secrets");
        secrets::table
            .first::<Secret>(&mut conn)
            .await
            .expect("Couldn't initialize secrets - make sure code_migrations ran first")
    };
    ...
    let reqwest_client = Client::builder()
        ...
        .build()
        .map_err(|e| TinyBoardsError::from_message(500, &format!("Failed to build HTTP client: {}", e)))?;
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            // Auth REST endpoints (login, register, refresh, logout, etc.)
            .configure(configure_auth_routes_with_secret(secret.jwt_secret.clone()))
