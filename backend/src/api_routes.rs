...
fn get_auth(req: &HttpRequest) -> Option<String> {
    ...
    // Fall back to tb_access cookie (httpOnly cookie set by auth handlers)
    if let Some(cookie) = req.cookie("tb_access") {
        return Some(format!("Bearer {}", cookie.value()));
    }
...
async fn perform_graphql(
    ...
    let logged_in_user =
        get_user_from_header_opt(context.pool(), context.master_key(), auth_header.as_deref()).await?;
