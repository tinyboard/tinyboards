...
#### 2. Configure TinyBoards

All configuration lives in a single `tinyboards.hjson` file at the project root. This is the single source of truth for the entire stack — both the backend and frontend read from it directly.
...
```hjson
{
  ...
  database: {
    user: "tinyboards"
    password: "your_very_secure_database_password_here"
...
For Docker deployments, run `./configure.sh` to generate the `.env` file that docker-compose needs from your `tinyboards.hjson`.
