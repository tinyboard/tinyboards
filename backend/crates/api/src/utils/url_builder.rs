...
mod tests {
    ...
    fn test_build_thread_url_with_boards() {
        ...
        let url = builder.build_thread_url(test_uuid(), "hello-world", Some("general"));
        assert!(url.starts_with("/b/general/threads/"));
        assert!(url.ends_with("/hello-world"));
    }

    #[test]
    fn test_build_thread_url_without_boards() {
        let builder = UrlBuilder::new(false);
        let url = builder.build_thread_url(test_uuid(), "hello-world", None);
        assert!(url.starts_with("/threads/"));
        assert!(url.ends_with("/hello-world"));
    }
    ...
    fn test_build_feed_url_with_boards() {
        let builder = UrlBuilder::new(true);
        let url = builder.build_feed_url(test_uuid(), "my-post", Some("tech"));
        assert!(url.starts_with("/b/tech/feed/"));
        assert!(url.ends_with("/my-post"));
    }
