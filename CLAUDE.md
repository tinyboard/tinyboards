...
### 2026-03-29 — Session 10: Board Modes Refactor
- **Replaced section bitmask with board_mode enum**: Removed `section_config` (Int4 bitmask), `section_order`, and `default_section` columns from boards table. Added `mode` column using new `board_mode` PostgreSQL enum ('feed' or 'forum').
- **Migration 000022_board_modes**: Creates enum type, migrates data (threads-only boards → forum, everything else → feed), drops old columns, adds `default_board_mode` to site table.
- **Backend**: Updated Diesel schema.rs, added `DbBoardMode` enum via `pg_enum!`, updated Board/Site models and insert/update forms, updated all GraphQL types and mutations (create board, update board settings, submit post, update site config), updated post validation (feed boards reject threads, forum boards reject non-threads).
- **Frontend**: Updated both GraphQL schemas, regenerated types, updated useBoard composable, board create/settings pages with two-card mode selector, board index page with mode-aware rendering (forum thread list vs feed post list), submit pages with auto-derived post type, board directory with mode filter (All/Feed/Forum), admin board settings with default mode selector, BoardTabs/BoardCard/BoardHeader/BoardPageSidebar/FeedSidebar components with mode badges/icons, AppNav with dynamic Feed Posts/Discussions links for mixed-mode subscriptions, auth store and SSR middleware to carry board mode data.
...
### 2026-04-28 — Messaging/Inbox Bug Fixes
- **Runtime error fixed**: `Unknown field "creatorId" on type "PrivateMessage". Did you mean "creator"?` — the GraphQL schema docs declared `creatorId: ID!` and `recipientId: ID` on `PrivateMessage`, and the frontend queried both, but the Rust struct in `backend/crates/api/src/structs/message.rs` had them marked `#[graphql(skip)]`. Exposed them as `creatorId: ID` and `recipientId: Option<ID>` on the GraphQL type and populated them in the `From<DbPrivateMessage>` conversion.
- **Conversation ordering**: `getConversation` returned messages in `created_at.desc()` order, so the frontend rendered newest-at-top and `scrollToBottom()` snapped to the oldest message. Backend now reverses each page to ASC (oldest -> newest within a page) so the rendered thread reads top-to-bottom chronologically and new messages append naturally.
- **`loadOlder` pagination**: Updated the look-ahead-extra slicing in `pages/inbox/messages/[id].vue` so prepending older pages keeps the array fully ASC end-to-end (`slice(-limit)` instead of `slice(0, limit)`).
- **Schema doc parity**: Added `markMessagesRead(messageIds: [ID!]!): Boolean!` to both `schema.graphql` files (the resolver already existed on the backend). Synced `frontend/schema.graphql`'s `PrivateMessage` type with the runtime — added `bodyHTML`, `isSenderHidden`, `isDeleted`, `creator`, `recipient`.

### 2026-04-28 — DM Composer: Emoji Picker, Autocomplete, Linked Mentions
- **Backend**: Added `convert_board_mentions_to_links` in `backend/crates/api/src/helpers/notifications.rs` — converts `b/<name>` references to `<a href="/b/name" class="board-mention">b/name</a>`. The regex requires a leading whitespace, opening tag (`>`), or `(`/`[` boundary, so URL paths like `https://example.com/b/foo` are not rewritten. Wired into `process_content_with_emojis` (`utils/emoji.rs`) so the new linkifier runs after the existing user-mention pass, meaning DMs, comments, and posts all gain `b/board` linkification (alongside the legacy `+board` syntax preserved in `custom_body_parsing`).
- **No DM ping side effects**: `send_message.rs` deliberately does not call `create_comment_mention_notification` / `create_post_mention_notification`, so user mentions in DMs render as profile links without sending the recipient a separate "you were mentioned" notification — only the existing `PrivateMessage` notification fires, matching the requested behavior.
- **Frontend EmojiPicker**: Rewrote `components/emoji/EmojiPicker.vue` from a stub into a real picker — search input, a Custom section that lazy-loads via `useEmoji.fetchAllAvailableEmojis`, and a Standard section sourced from `data/emojis.ts`. Picking a unicode emoji emits the unicode character; picking a custom emoji emits a `:shortcode:` token (which the backend expands to an `<img class="emoji">` during render).
- **Frontend MessageComposer**: Replaced the single-line input with an auto-resizing textarea (max 160px). Wired up `useEditorAutocomplete` so typing `:shortcode`, `@username`, or `b/boardname` opens the existing `EditorAutocompleteDropdown` with arrow-key navigation, Tab/Enter to accept, Escape to cancel. Added an emoji button beside the textarea that toggles the picker as a popover above the composer; click-outside dismisses. Enter sends, Shift+Enter inserts a newline.
- **Conversation page**: `pages/inbox/messages/[id].vue` now queries `bodyHTML` alongside `body` and renders it through `sanitizeHtml(message.bodyHTML || message.body)` (DOMPurify on the client, backend ammonia on SSR). Added scoped styles for compact paragraph margins, inline emoji image sizing, and underlined links that stay legible against the primary-colored own-message bubble.

### 2026-04-29 — Forum Polish + Reaction Persistence + Members Tab Counts
- **Forum board: participants column**: Forum thread list (`pages/b/[board]/index.vue`) now has a dedicated "Participants" column between Topic and Replies. Avatars upgraded from `xs` (20px) to `sm` (28px) with a tighter `-ml-2` overlap so they read well at the new size. Header grid changed to `1fr 140px 80px 120px`; each row gains a `.forum-thread-participants-col` (140px wide, centered). Hidden under 640px viewport; the legacy `.forum-thread-meta-row` flex wrapper was removed since participants no longer share space with the meta line.
- **Custom emoji size**: Bumped `img.emoji` in `assets/css/main.css` from `1.25em` to `1.875em` (Discord-like) with `vertical-align: middle`. Mirrored in `pages/inbox/messages/[id].vue` `.message-body :deep(img.emoji)`. Reaction bar (`components/common/ReactionBar.vue`) custom emoji icons grew to `w-6 h-6` for chips and `w-8 h-8` in the picker; picker buttons grew to 40-44px with a 7-column grid. Editor emoji picker (`components/editor/EmojiPicker.vue`) buttons 34→40px, custom images 24→30px, grid 8→7 columns. Added optional `.emoji-jumbo` modifier (3em) for future "emoji-only message" detection.
- **Reactions disappearing on refresh** — root cause was twofold:
  1. `POST_QUERY` and `COMMENTS_QUERY` never selected `reactionCounts` or `myReaction`, so on page render the `CommonReactionBar` always started with an empty array. Added the fields to `pages/b/[board]/[id]/[[slug]].vue` and `composables/useComments.ts`.
  2. `ReactionBar.vue` had no way to receive existing reactions. Added `initialReactions` and `myReactionEmoji` props plus a `setInitial(...)` exported from `useReactions` that builds the chip list and marks the current user's emoji as `reacted: true`. Threaded the props through `PostDetail.vue`, `ThreadDetailHeader.vue`, and `ThreadPost.vue`.
  3. **Bonus DB fix**: the `trg_reaction_aggregates` trigger (migration 008) used `INSERT ... ON CONFLICT DO NOTHING` against a table with **no** unique constraint, so every reaction inserted a new aggregate row AND ran `UPDATE ... SET count = count + 1` against every matching row — counts inflated and aggregates duplicated. Migration `2026-04-29-000025_fix_reaction_aggregates` truncates and rebuilds aggregates from the `reactions` table, adds proper UNIQUE partial indexes on `(post_id, emoji)` and `(comment_id, emoji)`, and replaces the trigger with `UPDATE` + `IF NOT FOUND THEN INSERT` semantics.
- **Members tab — Top Contributors counts**: `BoardContributor` GraphQL type changed from `{postScore, commentScore, totalScore}` to `{postCount, commentCount}`. Backend (`queries/board_members.rs`) now counts non-removed/non-deleted posts and comments (last 30 days) instead of summing aggregate scores. Synced both `schema.graphql` files and `frontend/types/generated.ts`. Frontend (`pages/b/[board]/members.vue`) shows the count and labels them per board mode: forum→"thread(s)", feed→"post(s)"; comments are always rendered as "reply/replies" per request. Total-score column removed.
