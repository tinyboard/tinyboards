...
use tinyboards_db::{
    models::{
        board::boards::BoardSafe, moderator::mod_actions::ModStickyPost, post::posts::Post, person::person::PersonSafe,
    },
    schema::{boards, mod_sticky_post, posts, person},
    traits::{ToSafe, ViewToVec},
    utils::{limit_and_offset, DbPool, get_conn},
};
use diesel_async::RunQueryDsl;

type ModStickyPostViewTuple = (ModStickyPost, Option<PersonSafe>, Post, BoardSafe);
...
impl ModStickyPostView {
    pub async fn list(pool: &DbPool, params: ModLogParams) -> Result<Vec<Self>, Error> {
        ...
        let mod_names_join = mod_sticky_post::mod_person_id
            .eq(person::id)
