use crate::structs::{ModAddView, ModLogParams};
...
use tinyboards_db::{
    ...
    models::{
        user::user::UserSafe,
        moderator::mod_actions::ModAdd,
    },
    traits::{ToSafe, ViewToVec},
    utils::limit_and_offset,
};

type ModAddViewTuple = (
    ModAdd,
    Option<UserSafe>,
    UserSafe,
);
...
impl ModAddView {
    pub fn list(conn: &mut PgConnection, params: ModLogParams) -> Result<Vec<Self>, Error> {
        ...
        let mod_names_join = mod_add::mod_user_id
            .eq(user_::id)
        ...
        let mut query = mod_add::table
            ...
            .inner_join(
                user_alias.on(mod_add::other_user_id.eq(user_alias.field(user_::id))),
            )
            .select((
                mod_add::all_columns,
        ...
        if let Some(other_user_id) = params.other_user_id {
            query = query.filter(mod_add::other_user_id.eq(other_user_id));
        };
