...
use tinyboards_db::{
    models::{moderator::admin_actions::AdminPurgePost, person::person::PersonSafe},
    schema::{admin_purge_post, person},
    traits::{ToSafe, ViewToVec},
    utils::{limit_and_offset, DbPool, get_conn},
};
use diesel_async::RunQueryDsl;

type AdminPurgePostViewTuple = (AdminPurgePost, Option<PersonSafe>);
...
impl AdminPurgePostView {
    pub async fn list(pool: &DbPool, params: ModLogParams) -> Result<Vec<Self>, Error> {
        ...
        if let Some(admin_person_id) = params.mod_person_id {
            query = query.filter(admin_purge_post::admin_id.eq(admin_person_id));
        };
