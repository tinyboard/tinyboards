...
use tinyboards_utils::{settings::structs::Settings, TinyBoardsError};
use url::Url;
...
static CONTEXT: Lazy<Vec<serde_json::Value>> = Lazy::new(|| {
    serde_json::from_str(include_str!("../assets/tinyboards/context.json"))
        .expect("parse apub context")
});
...
fn check_ap_id_valid(ap_id: &Url, local_site_data: &LocalSiteData) -> Result<(), &'static str> {
...
pub(crate) fn check_ap_id_valid_with_strictness(
    ...
    if is_strict && !local_site_data.allowed_instances.is_empty() {
        ...
        let domain = ap_id.domain().expect("apub id has domain").to_string();

        if !allowed_and_local.contains(&domain) {
            return Err(TinyBoardsError::from_message(
                403,
                "federation forbidden by a strict allow list.",
            ));
        }
    }
    Ok(())
