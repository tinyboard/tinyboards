...
use tinyboards_api_common::{
    ...
    post::{CreatePostVote, PostResponse},
    utils::require_user,
};
use tinyboards_db::{
    models::{
...
impl SendActivity for CreatePostVote {
    ...
    async fn send_activity(
        ...
        let object_id = ObjectId::from(response.post_view.post.ap_id.clone().unwrap());
        let board_id = response.post_view.board.id;
        let view = require_user(context.pool(), context.master_key(), auth)
            .await
            .unwrap()?;
        send_activity(object_id, board_id, request.score, view, context).await
    }
}
...
impl SendActivity for CreateCommentVote {
    ...
    async fn send_activity(
        ...
        let object_id = ObjectId::from(response.comment_view.comment.ap_id.clone().unwrap());
...
async fn send_activity(
    ...
    let actor = Person::read(context.pool(), view.person.id).await?.into();
