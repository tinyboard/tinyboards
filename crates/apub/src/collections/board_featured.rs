...
use tinyboards_api_common::{data::TinyBoardsContext, utils::generate_featured_url};
use tinyboards_db::{models::post::posts::Post, utils::FETCH_LIMIT_MAX};
use tinyboards_utils::error::TinyBoardsError;
use url::Url;

#[derive(Clone, Debug)]
pub(crate) struct ApubBoardFeatured(Vec<ApubPost>);

#[async_trait::async_trait]
impl Collection for ApubBoardFeatured {
    ...
    async fn read_local(
        ...
        let ordered_items = try_join_all(
          Post::list_featured_for_board(data.pool(), owner.id)
            .await?
            .into_iter()
            .map(ApubPost::from)
