...
/// let subject = "acct:kroner@tinyboards.net".to_string();
/// let url = Url::parse("https://tinyboards.net/@kroner")?;
...
/// let subject = "acct:kroner@tinyboards.net".to_string();
/// let user = Url::parse("https://tinyboards.net/@kroner")?;
/// let group = Url::parse("https://tinyboards.net/+general")?;
...
pub struct Webfinger {
    /// The actor which is described here, for example `acct:kroner@tinyboards.net`
    pub subject: String,
...
mod tests {
    ...
    async fn test_webfinger() {
        ...
        let res =
            webfinger_resolve_actor::<DbConnection, DbUser>("kroner@tinyboards.net", &data)
                .await;
        assert!(res.is_ok());
    }
}
