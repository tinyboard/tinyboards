...
use tinyboards_db_views::structs::{CommentReplyView, MessageView, PersonMentionView};
...
impl<'des> Perform<'des> for GetUnreadCount {
    ...
    async fn perform(
        ...
        let person = require_user(context.pool(), context.master_key(), auth)
            .await
            .unwrap()?
            .person;
        ...
        let messages = MessageView::get_unread_count(context.pool(), person_id).await?;
