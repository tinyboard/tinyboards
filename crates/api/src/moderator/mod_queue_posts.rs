...
use tinyboards_api_common::{post::GetPostsResponse, moderator::PostModQueue};
use crate::Perform;
...
impl<'des> Perform<'des> for PostModQueue {
    ...
    async fn perform(
    	...
    	let PostModQueryResponse { posts, count } = PostModQuery::builder()
    		...
    		.list()
    		.await
    		.map_err(|e| TinyBoardsError::from_error_message(e, 500, "Couldn't load reported posts."))?;

    	Ok(GetPostsResponse { posts, total_count: count })
    }
}
