...
use tinyboards_api_common::{
    admin::PurgeUser,
    utils::{blocking, require_user, purge_image_posts_for_user}, 
...
impl<'des> Perform<'des> for PurgeUser {
    ...
    async fn perform(
        ...
        if target_user.name.to_lowercase().contains(settings.owner_name.to_lowercase().as_str()) || target_user.admin {
            return Err(TinyBoardsError::from_message("can't purge a site admin or the owner"));
        }

        // purge user banner
        if let Some(banner) = target_user.banner {
            purge_image_from_pictrs(context.client(), context.settings(), &Url::parse(banner.as_str()).unwrap())
            .await
            .ok();
        }
        ...
        if let Some(avatar) = target_user.avatar {
            purge_image_from_pictrs(context.client(), context.settings(), &Url::parse(avatar.as_str()).unwrap())
            .await
            .ok();
        }
