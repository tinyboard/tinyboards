use crate::{check_report_reason, PerformCrud};
use actix_web::web::Data;
use tinyboards_api_common::{
    data::TinyBoardsContext,
    post::{CreatePostReport, PostReportResponse},
    utils::{/*send_new_report_email_to_admins,*/ check_board_ban, require_user},
};
use tinyboards_db::{
    models::{
...
impl<'des> PerformCrud<'des> for CreatePostReport {
    ...
    async fn perform(
        ...
        let post_report_view =
            PostReportView::read(context.pool(), report.id, Some(person_id)).await?;
