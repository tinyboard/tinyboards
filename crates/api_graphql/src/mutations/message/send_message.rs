...
impl SendMessageMutations {
    ...
    pub async fn send_message(
        ...
        let is_blocked = PersonBlock::read(pool, input.recipient_id, user.person.id).await.is_ok();
        if is_blocked {
            return Err(TinyBoardsError::from_message(403, "You are blocked by this user").into());
        }

        // Check if recipient has blocked sender
        let has_blocked = PersonBlock::read(pool, user.person.id, input.recipient_id).await.is_ok();
        if has_blocked {
