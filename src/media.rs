...
use tinyboards_db::{models::site::{site::Site, stray_images::StrayImage}};
...
async fn upload_image_file(
    ...
    if let Some(files) = &images.files {
        ...
        // add image url to stray image cache, allow to soft fail
        StrayImage::add_url_to_stray_images(context.pool(), image_url.clone()).await.ok();
    }
