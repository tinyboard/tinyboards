...
const { execute, loading: postLoading, error: postError } = useGraphQL<ThreadResponse>()
...
async function fetchPost (): Promise<void> {
  const result = await execute(THREAD_QUERY, { variables: { id: threadId } })
  if (result?.post) {
...
// Must be called in synchronous setup context
useHead({ title: computed(() => post.value?.title ?? 'Thread') })
useSeoMeta({
  title: computed(() => post.value?.title ?? 'Thread'),
  ogTitle: computed(() => post.value?.title ?? 'Thread'),
  description: computed(() => {
    if (!post.value?.body) return `Thread in +${boardName}`
...
// Redirect feed posts to the correct URL
if (post.value && !post.value.isThread) {
  const boardName = post.value.board?.name ?? (route.params.board as string)
  const slug = post.value.slug ?? ''
  const slugPart = slug ? `/${slug}` : ''
  await navigateTo(`/b/${boardName}/feed/${threadId}${slugPart}`, { replace: true })
}
...
// Flatten all comments into a linear list sorted by creation time
...
async function handleTopLevelComment (body: string): Promise<void> {
  const newComment = await submitComment(threadId, body)
  if (newComment) {
    await fetchComments(threadId)
  }
}
...
<template>
  <div>
    ...
    <template v-else-if="post">
      ...
      <!-- Linear comment list (forum-style) -->
      <div class="space-y-3">
        <CommonLoadingSpinner v-if="commentsLoading && comments.length === 0" />
        <CommonErrorDisplay v-if="commentsError" :message="commentsError.message" @retry="fetchComments(threadId)" />
